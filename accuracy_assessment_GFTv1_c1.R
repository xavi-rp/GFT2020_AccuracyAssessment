...
## using the 1st version of the validation data set (Collection1 -c1-).
...
table(Valid_c1_GFT2020_V1$type_class)  # 
# Naturally regenerating forest    no trees or shrubs present      other wooded land       Planted or plantation forest    trees for agricultural use 
# 6337                             10357                           2035                    256                             284 
# trees in urban areas          trees inside forest          trees outside forest 
# 618                           446                          1279  

nrow(Valid_c1_GFT2020_V1)  # 21612
table(Valid_c1_GFT2020_V1$forest_class)
#     Forest  Non-forest 
#       6593       15019 (non-forest in the validation dataset, but forest in GFT)



## Merging with those rows from the Validation dataset that are not exported from GEE, and therefore Non-forest in GFT2020v1 
## In this case, that's already done earlier; just keeping the "_full" for consistency with the code

Valid_c1_GFT2020_V1_full <- Valid_c1_GFT2020_V1
#Valid_c1_GFT2020_V1_full <- valid_c1 %>%
#  left_join(
#    Valid_c1_GFT2020_V1 %>%
...
head(Valid_c1_GFT2020_V1_full)
table(Valid_c1_GFT2020_V1_full$GFT2020_V1)    # 1: Naturally regenerating forest; 10: Primary forest; 20: Planted/plantation forest
#      1    10   20 
#   3571  2527  627 
sum(table(Valid_c1_GFT2020_V1_full$GFT2020_V1))   # 6725  Forest in GFT
sum(is.na(Valid_c1_GFT2020_V1_full$GFT2020_V1))   # 14887 Non-forest in GFT


## Clement's numbers:
6084 + 629   # = 6713 (total forest: natural + planted)
14899        # Non-Forest
...
Valid_c1_GFT2020_V1_full <- Valid_c1_GFT2020_V1_full %>%
  mutate(
    GFT_forest = if_else(
      is.na(GFT2020_V1),
      "Non-Forest",
...
head(Valid_c1_GFT2020_V1_full)
table(Valid_c1_GFT2020_V1_full$GFT_forest)
#     Forest   Non-Forest 
#       6725        14887 

sum(is.na(Valid_c1_GFT2020_V1_full$GFT_forest)) # 0
...
cm <- Valid_c1_GFT2020_V1_full %>% 
  count(forest_class, GFT_forest) %>%
  rename(ValidC1_forest_class = forest_class) #%>% 
  #pivot_wider(
...
p <- ggplot(cm, aes(x = GFT_forest, y = ValidC1_forest_class, fill = n)) +
  geom_tile() +
...
sve_it <- "no"
if(sve_it == "yes"){
  ggsave(paste0(dir_GFTv1_assessment_c1, "Valid_C1_GFT2020_V1_Forest_NonForest_confusion_matrix.png"), 
         p, width = 7, height = 6, dpi = 300)
}

                         # Ref(C1)  GFTv1
5261 + 66 + 410 + 137    # Forest - Forest:  5874 (Clement)   (xavi) 5874
                         # NonFor - NonFor: 14180 (Clement)   (xavi) 14168 *  (-12)
666 + 53                 # forest - NonFor:   719 (Clement)   (xavi) 719
757 + 82                 # NonFor - Forest:   839 (Clement)   (xavi) 851 *    (+12)
...
scenario <- Valid_c1_GFT2020_V1_full %>% 
...
head(scenario)
table(Valid_c1_GFT2020_V1_full$forest_class)
table(scenario$forest_class_num)
#     0     1 
# 15019  6593 


names(scenario)   # "forest_class_num" "type_class"       "strata"           "GFT2020_V1"       "gaul"    
nrow(scenario)   # 21612
...
## Overall accuracy
scenario_AA$OA   # 0.9123273
round(scenario_AA$OA, 3)   # 0.912
...
#    0.9548987       0.8529271       0.4267526   (c2)
#    0.9540562       0.8565953       0.2359441


1 - scenario_AA$UA#[2]

# 0            1            2 
# 0.04510134   0.14707287   0.57324737      (c2)
# 0.04594378   0.14340469   0.76405591

# Commission Error (Non-forest) =   0.046         (c2: 0.045)
# Commission Error (Natural)    =   0.143         (c2: 0.147)
# Commission Error (Planted)    =   0.764         (c2: 0.573)
# The estimated commission error for the Naturally regenerating Forest class was 14.3%, indicating that  
# approximately 14% of the area mapped as Naturally regenerating Forest is estimated to correspond  
# to other reference classes (Non-forest or Planted/plantation forest).

# More than half of the area mapped by GFT as Planted/plantation (76.4%) is estimated to belong to another reference class.
...
#         0           1            2
# 0.9433225   0.8672492    0.5314436  (c2)
# 0.9424067   0.8558813    0.4429151


print(paste0("Ommission Error (Non-forest) = ",
             round((1 - scenario_AA$PA[1]), 3)))  # An estimated 5.8% of the pixels that are truly Non-forest were mapped as one of the forest classes.


print(paste0("Ommission Error (Naturally Regenerating Forest) = ",
             round((1 - scenario_AA$PA[2]), 3)))  # An estimated 14.4% of the pixels that are truly Naturally Regenerating Forest were mapped as
                                                  # either  Planted Forest or Non-Forest.

print(paste0("Ommission Error (Planted / Plantation Forest) = ",
             round((1 - scenario_AA$PA[3]), 3)))  # An estimated 55.7% of the pixels that are truly Planted / Plantation Forest were mapped as
...
scenario_AA$SEoa    #  0.002263458          0.002206576 (c2)
...
scenario_AA$SEua
#           0             1             2
# 0.002035794   0.005266056   0.023853870  (c2)
# 0.002142697   0.005260284   0.021720480 


## standard error of PA
scenario_AA$SEpa
#           0             1             2
# 0.002195413   0.005055741   0.027248449    (c2)
# 0.002192096   0.005187289   0.037522123 


## Proportion of area 
scenario_AA$area
#         0             1              2
# 0.70013412   0.28475867     0.01510722   (c2)
# 0.70019622   0.28978179     0.01002199 


## standard error of area proportion
scenario_AA$SEa
#            0               1               2
#  0.003123383     0.003097202     0.000791796   (c2)
#  0.0031581230    0.0031410035    0.0007585244 
...
#                                 Reference (validation)   (c2)     !!    
#                        0           1            2         sum     !!                    0           1            2         sum  
#    G |  0    0.660452257  0.02895764  0.002236540  0.69164644     !!    0     0.659869584  0.02937431  0.002402546  0.69164644
#    F |  1    0.037741473  0.24695673  0.004842043  0.28954025     !!    1     0.038340878  0.24801882  0.003180553  0.28954025
#    T |  2    0.001940385  0.00884430  0.008028633  0.01881332     !!    2     0.001985758  0.01238867  0.004438891  0.01881332
#         sum  0.700134115  0.28475867  0.015107215  1.00000000     !!    sum   0.700196220  0.28978179  0.010021991  1.00000000
...
# UA (planted) =  %;  Commission Error (Planted) =  %
# Total area mapped as Planted:  0.001985758 + 0.01238867 + 0.004438891 = 0.01881332
...
#          0           1           2
#  0.3990156   1.0321469   4.6753586   (c2)
#  0.4199686   1.0310158   4.2572141 


CI95_PA <- z * scenario_AA$SEpa * 100; CI95_PA
#          0           1           2
#  0.4303009   0.9909252   5.3406960  (c2) 
#  0.4296508   1.0167086   7.3543362 
...
Valid_c1 <- map_ref_values   # change the name to write correct file/tab names

res <- report_error_matrix(
  #data       = Valid_c2,             # change the name
  data       = Valid_c1,             # change the name
  map_col    = "GFT_type",             
  ref_col    = "forest_type",
  assessment = scenario_AA,
  #output_dir = dir_GFTv1_assessment_c2     # change the name
  output_dir = dir_GFTv1_assessment_c1,     # change the name
  export_file = "no"
)
...
figure_title <- "GFT2020_v1 / Valid_c1"
...
plot_error_map <- function(pts, world, figure_title = NULL) {
  ...
  class_sizes <- c(
    "Natural Forest" = 0.40,
    "Planted Forest" = 0.40,
    "Commission error (Natural Forest)" = 0.6,
    "Commission error (Planted Forest)" = 0.6,
    "Omission error (Natural Forest)" = 0.6,
    "Omission error (Planted Forest)" = 0.6
  )
...
pts_natural <- pts %>% 
  ...
  filter(class %in% c("Commission error (Natural Forest)", "Omission error (Natural Forest)"))
